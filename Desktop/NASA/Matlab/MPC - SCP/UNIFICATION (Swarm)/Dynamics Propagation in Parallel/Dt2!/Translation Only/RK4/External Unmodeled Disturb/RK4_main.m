%% MPC - SCP of ROBOTIC SWARM (DETERMINISTIC APPROACH) - Translation + Attitude
...
% MPC Time Horizon (It's a number of steps to be done at each optimization)
...
sol = ode45 ( @(t,x) odefcn_OE( t, x ), [t0 tf+Dt], x_OE_0 ); % CONTINUOUS TIME!
%% State Matrices
B = [ zeros(3)
      1/m*eye(3) ]; 
E = [ zeros(3)
...
Kp = 1e0*eye(3); 
Kd = 1e0*eye(3);  %  u = Kp x_e + Kd v_e(which is smaller, so need bigger Kd to counterwight)
%% If the control requires too much vel or force, and the optimiz becomes unfeasible
...
while ~isempty(K)  %length(K)~=0
    ...
    parfor ii = 1:length(K)            % exact robot indices 
%        Where xi=x(:,:,i)
...
% 3. After finishing the MPC calulation on the horizon, so after t_RUN,
...
% % %         end
    ...
    valuesXgap = length( tspan(1):Ts:tspan(end) );  % NOW valuesXgap CHANGES EACH TIME
...
% %     % Finish integrating the system up to T...
    if ceil((t_actualOld+t_run)/Dt)*Dt < T  
        ...
        valuesXgap = length( tspan(1):Ts:tspan(end) );  
        tt_temp=zeros(1,valuesXgap,N);
        xx_temp=zeros(state,valuesXgap,N); uu_temp=zeros(3,valuesXgap,N);
        timeline=tspan(1):Ts:tspan(end);
        endColumn=size(tt_save,2);
